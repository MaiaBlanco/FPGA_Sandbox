...
//#define TMR_RESET_VALUE	 	0xFFFFFC17 // This will cause the timer to overflow every 1 milliseconds
#define TMR_RESET_VALUE		0xFF000000//00BDBF // This will cause the timer to overflow every 10 milliseconds??
...
#define BRAM_SIZE_IN_WORDS ((XPAR_AXI_BRAM_CTRL_0_S_AXI_HIGHADDR-XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR) >> 2 )
...
static volatile u32 debounce_counter;
static char debouncing;
...
//static inline void debounce_btn(u32 millis);
static void wait_before_next_test();
...
//		// Update the LED array in case we are running a new test:
//		led_data = XGpio_DiscreteRead(&LEDInst, 1);
//		// Clear the lower 4 bits
//		led_data &= ~0xF;
//		// Write the updated test number to those bits
//		led_data |= ((test_number-1) & 0xF);
//		// Write the value back to the GPIO LEDs
//		XGpio_DiscreteWrite(&LEDInst, 1, (u32)led_data);
//
//		if (debouncing != 0)
//		{
...
int main()
{
    ...
    printf("Beginning test %d: Init timer, GPIO, and buttons with interrupts.\n\r",
    		test_number);
    ...
    {
    	Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + (i << 2), i);
    }
    ...
    for (i = 0; i <= BRAM_SIZE_IN_WORDS; i++)
    {
    	data = Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + (i << 2));
    	if ( data != i )
    	{
    ...
    print("Data written and read from BRAM successfully!\n\r");
    ...
    for (i = 0; i < 16384; i++)
    {
    	Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + (i << 2), i+k);
    }
    ...
    print("More data written and read successfully to BRAM.\n\r");
    ...
    k = BRAM_SIZE_IN_WORDS >> 3;
    XTime_GetTime(&t_start);
    for (i = 0; i < 0xFFFF; i ++)
    {
    	data = Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + (k << 2));
    }
    XTime_GetTime(&t_end);
    time_elapsed = ((double)(t_end-t_start) / (double)COUNTS_PER_SECOND);
    printf("Performed %u memory reads at address %u in %f seconds.\n\r",
    		0xFFFF, XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + (k << 2), time_elapsed);
    printf("Data read rate: %f B/s\n\r", (double)(0xFFFF << 2) / time_elapsed);
    ...
    for (i = 0; i < 0xFFFF; i ++)
    {
    	Xil_Out32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + (k << 2), data);
    }
    XTime_GetTime(&t_end);
    time_elapsed = ((double)(t_end-t_start) / (double)COUNTS_PER_SECOND);
    printf("Performed %u memory writes of %u at address %u in %f seconds.\n\r",
    		0xFFFF, data, XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + (k << 2), time_elapsed);
    ...
    XTime_GetTime(&t_start);
    for (i = BRAM_SIZE_IN_WORDS; i >= 0; i --)
    {
    	data = Xil_In32(XPAR_AXI_BRAM_CTRL_0_S_AXI_BASEADDR + (i << 2));
    }
    XTime_GetTime(&t_end);
...
// Delay for millis milliseconds (timer count has a resolution of 100 us)
//void debounce_btn(u32 millis)
//{
//	debounce_counter = 0;
//	debouncing = 1;
//	while(debounce_counter < (millis*10));
//	debouncing = 0;
//}

// Handle waiting before moving to the next test.
void wait_before_next_test()
{
    test_number ++;
    printf("Press middle button to start test %d.\n\r", test_number);
    while(0)
    {
    	if (btn_value == 1)
    	{
    	...
    	{
    		print("Other buttons do not have an assigned function. Press the middle button.\n\r");
    	}
    }
}
