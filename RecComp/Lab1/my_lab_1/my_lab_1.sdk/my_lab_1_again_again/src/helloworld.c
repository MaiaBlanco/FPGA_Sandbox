...
#define TMR_RESET_VALUE	 	0xFFF0BDBF	// The timer will tick every 0.01 seconds
#define TIMER_CNTR_0	 	0
#define XPAR_TMRCTR_0_CLOCK_FREQ_HZ XPAR_AXI_TIMER_0_CLOCK_FREQ_HZ
#define BTN_INT 			XGPIO_IR_CH1_MASK
#define TIMER_COUNT_LIM 	100000
...
void TMR_Intr_Handler(void *data)
{
	if (XTmrCtr_IsExpired(&timer_instance, 0))
	{
		...
		if (debouncing != 0)	debounce_timer++;
		// Update the low 4 LEDs to the current test count
		led_data = XGpio_DiscreteRead(&LEDInst, 1);
		led_data = led_data >> 4;
		led_data = led_data << 4;
		led_data |= (0xF & test_number);
		XGpio_DiscreteWrite(&LEDInst, 1, (u32)led_data);
	}
}
...
int main()
{
    ...
    for (i = 0; i < BRAM_SIZE_IN_WORDS; i++)
    {
    	...
    	if ( data != i )
    	{
    		printf("ERROR: value read was %u instead of %u.\n\r",
    				data, i);
    		print("Quitting this test and moving on.\n\r");
    		break;
    	}
    }
    ...
    XTime_GetTime(&t_end);
    time_elapsed = ((double)(t_end-t_start) / (double)COUNTS_PER_SECOND);
    printf("Performed %u memory writes at increasing sequential addresses in %f seconds.\n\r",
    		BRAM_SIZE_IN_WORDS, time_elapsed);
    printf("Data read rate: %f B/s\n\r", (BRAM_SIZE_IN_WORDS << 2) / time_elapsed);
    XTime_GetTime(&t_start);
    for (i = BRAM_SIZE_IN_WORDS-1; i >= 0; i --)
    ...
    // Alternatively, read from one address many times, and then start reading with stride over the whole BRAM.
    // If the read bandwidth doesn't change, then we are not caching.
    print("Starting test 7.\n\r");
    XTime_GetTime(&t_start);
    for (i = 0; i <= (BRAM_SIZE_IN_WORDS >> 5) * 20; i ++)
    {
    	data = Xil_In32(XPAR_BRAM_0_BASEADDR);
    }
    XTime_GetTime(&t_end);
    time_elapsed = ((double)(t_end-t_start) / (double)COUNTS_PER_SECOND);
    printf("Performed %u memory reads at same addresses in %f seconds.\n\r",
    		(BRAM_SIZE_IN_WORDS >> 5) * 20, time_elapsed);
    printf("Data read rate: %f B/s\n\r", (double)(BRAM_SIZE_IN_WORDS >> 5) * 20 / time_elapsed);

    // Stride by 8 32-bit entries
    time_elapsed = 0;
    for (k = 0; k < 20; k ++)
    {
    	XTime_GetTime(&t_start);
    	for (i = 0; i <= BRAM_SIZE_IN_WORDS >> 5; i ++)
    	    {
    	    	data = Xil_In32(XPAR_BRAM_0_BASEADDR + (i << 2) + (k << 2));
    	    }
    	XTime_GetTime(&t_end);
    	time_elapsed +=  ((double)(t_end-t_start) / (double)COUNTS_PER_SECOND);
    }

    printf("Performed %u memory reads at strided addresses in %f seconds.\n\r",
    		(BRAM_SIZE_IN_WORDS >> 5) * 20, time_elapsed);
    printf("Data read rate: %f B/s\n\r", ((double)(BRAM_SIZE_IN_WORDS >> 5) * 20) / time_elapsed);
...
{
	while(btn_value != 0);
	printf("Press the middle button to continue to test %d.\n\r", test_number + 1);
	debounce();
	while(btn_value != 1);
	debounce();
	test_number ++;
